      ...
      <table class="task-table">
        ...
        <tbody>
          <tr v-for="m in filteredModels" :key="m.modelProfileId" :class="{ 'row-disabled': !m.enabled }">
            ...
            <td>{{ m.apiKeyRef || '-' }}</td>
            <td>
              <button class="link-btn" @click="pingOne(m)">Ping</button>
        ...
        <div class="form-layout">
          <label>
            ...
            <select v-model="createForm.role" @change="onRoleChange">
              <option value="PLAYER">PLAYER（受评模型）</option>
              <option value="JUDGE">JUDGE（评判模型）</option>
              <option value="BOTH">BOTH（两者通用）</option>
            </select>
          </label>

          <!-- modelId：下拉 + 自定义 -->
          <label v-if="catalog && availableCatalogItems.length">
            modelId（从推荐清单选择，或选「自定义」手动输入）
          ...
          <label>
            显示名
            <input v-model.trim="createForm.displayName" type="text" placeholder="如 Qwen3-32B Player" />
          </label>
          <label>
            apiKeyRef（环境变量名，留空走全局 llm.api-key）
            <input v-model.trim="createForm.apiKeyRef" type="text" placeholder="MODELSCOPE_API_KEY" />
          </label>
          <label>
            apiBaseUrl（留空走全局 base-url）
...
async function submitCreate(): Promise<void> {
  ...
  if (!finalModelId) {
    notice.value = 'modelId 必填（请从下拉选择，或手动输入）';
    return;
  }
  if (!createForm.displayName) {
    notice.value = '显示名必填';
  ...
  if (existing) {
    if (existing.role === createForm.role || existing.role === 'BOTH') {
      notice.value = `已存在同 modelId 的 ${existing.role} 角色（#${existing.modelProfileId}），无需重复注册`;
      return;
    }
    // 已注册为另一角色 → 询问用户是否升级为 BOTH
    const yes = window.confirm(
      `「${finalModelId}」已注册为 ${existing.role} 角色（#${existing.modelProfileId}）。\n\n` +
        `同一 modelId 不能注册两次（DB 唯一约束）。\n` +
        `是否把它的角色升级为 BOTH（同时支持 PLAYER 和 JUDGE）？`,
    );
    if (!yes) return;
    try {
...
async function pingOne(m: ModelProfile): Promise<void> {
  notice.value = `正在 ping ${m.modelId}...`;
  try {
    const r = await pingModel(m.modelProfileId);
    const preview = r.text.length > 60 ? r.text.slice(0, 60) + '...' : r.text;
    notice.value = `[${m.modelId}] ${r.text.length}c · tokens=${r.promptTokens}/${r.completionTokens} · ${r.latencyMs}ms · ${preview || '(空)'}`;
  } catch (err: any) {
    notice.value = `ping 失败 [${m.modelId}]: ${err.message || String(err)}`;
  }
}
