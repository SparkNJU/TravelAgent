...
- 排行榜页面
  ...
  - 职责：拉取并渲染全部模型评分、胜负平统计详情，支持搜索与条件筛选。
...
排行榜条目支持可折叠的详细信息展示，字段为可选：

- meta.vendor：模型厂商
- meta.family：模型系列
- meta.contextWindow：上下文窗口
- meta.tags：标签
- meta.description：说明

说明：前端不会写死厂商/系列信息，字段若不存在则展示为“未知/空”。元信息来源于 Agent 侧模型注册表接口（/api/models），由后端转发到排行榜条目。
